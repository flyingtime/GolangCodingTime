...
所谓的**静态类型**（也即 static
type）是你在编码是看见的类型(如 int、string)，如
...
比如 `var age : 25` 这条语句，value 是 25，type 是 int

pair 的存在，是 Golang 中实现反射的前提，理解了 pair，就更容易理解反射。
...
SetFloat()（或者 SetInt 、SetString 等其他类似的函数）还应该使用 Elem
函数
...
[第 001 节：反射 reflect](https://www.qfgolang.com/?special=fanshejizhi)
